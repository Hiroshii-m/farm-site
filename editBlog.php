...
if(!empty($_POST)) {
    ...
    if(empty($err_msg)) {
        try {
            // ブログを登録
            $dbh = dbConnect();
            $sql = 'UPDATE `blogs` SET `shop_id` = :s_id, `title` = :title, `img` = :img, `content` = :content, `create_date` = :create_date';
...
?>
    <body>
        ...
        <main id="l-main">
            <!-- パンクズリスト -->
            <div class="c-pankuzu">
                <a href="myBlogList.php<?= appendGetParam(); ?>" class="u-prev p-article__prev">戻る</a>
            </div><!-- /パンクズリスト -->
