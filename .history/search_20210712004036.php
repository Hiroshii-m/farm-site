                            ...
                            <select class="p-search__select" name="city_id" id="">
                                <?php if(!empty(array_filter($cityInfo, "array_filter"))){ ?>
                                    <?php foreach($cityInfo as $key => $val): ?>
                                        <?php debug(print_r($val, true)); ?>
                                        <option value="<?= sanitize(showData($val['id'])); ?>" <?= (!empty($city_id) && (int)$city_id === $val['id']) ? 'selected' : ''; ?>><?= sanitize(showData($val['city_name'])); ?></option>
                                    <?php endforeach; ?>
                                <?php } ?>
                            </select>
                            ...
                            <select class="p-search__select" name="category_id" id="">
                                <?php if(!empty(array_filter($category, "array_filter"))){ ?>
                                    <?php foreach($category as $key => $val): ?>
                                        <option value="<?= sanitize($val['id']); ?>" <?= ((!empty($category_id) || (int)$category_id !== 0) && (int)$category_id === $val['id']) ? 'selected' : ''; ?>><?= sanitize(showData($val['category_name'])); ?></option>
                                    <?php endforeach; ?>
                                <?php } ?>
                            </select>
                            <i class="fas fa-chevron-down p-search__arrow"></i>
                    ...
                    <h2 class="p-shopList__heading">
                        ...
                        <div class="p-shopList__terms">
                            <?php if(!empty($_GET['city_id']) && is_numeric($_GET['city_id'])){ ?>
                                <span class="p-shopList__tag u-tag-sub"><?php //sanitize($cityInfo[$_GET['city_id']]['city_name']); ?></span>
                            <?php } ?>
