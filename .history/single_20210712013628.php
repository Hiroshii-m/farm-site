...
// 市区町村データ取得
$cityInfo = (!empty($dbFormData['prefecture_id'])) ? getCityInfo($dbFormData['prefecture_id']) : array();
// 住所を取得
$access = '';
...
if(!empty($dbFormData['prefecture_id'])){
    ...
    // if(!empty($dbFormData['city_id']) && !empty($cityInfo['city_id'])){
    //     // $access = $cityInfo['city_id'].$dbFormData['street'].$dbFormData['building'];
    // }
    foreach($cityInfo as $key => $val){
        if($val['id'] === $dbFormData['city_id']){
            $access .= $val['city_name'];
        }
    }
    $access .= $dbFormData['street'].$dbFormData['building'];
}
