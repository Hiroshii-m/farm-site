                            ...
                            <select class="p-search__select" name="city_id" id="">
                                <?php if(!empty(array_filter($cityInfo, "array_filter"))){ ?>
                                    <?php foreach($cityInfo as $key => $val): ?>
                                        <option value="<?= sanitize(showData($val['city_id'])); ?>" <?= ((!empty($city_id) || (int)$city_id === 0) && (int)$city_id === $key) ? 'selected' : ''; ?>><?= sanitize(showData($val['city_name'])); ?></option>
                                    <?php endforeach; ?>
                                <?php } ?>
                            </select>
                            ...
                            <select class="p-search__select" name="category_id" id="">
                                <?php if(!empty(array_filter($category, "array_filter"))){ ?>
                                    <?php foreach($category as $key => $val): ?>
                                        <option value="<?= sanitize($val['category_id']); ?>" <?= ((!empty($category_id) || (int)$category_id === 0) && (int)$category_id === $key) ? 'selected' : ''; ?>><?= sanitize(showData($val['category_name'])); ?></option>
                                    <?php endforeach; ?>
                                <?php } ?>
                            </select>
                            <i class="fas fa-chevron-down p-search__arrow"></i>
