...
function getShopMatch($currentMinNum = 0, $p_id, $city_id, $category_id, $word_search, $span = 10) {
    try {
        ...
        // 表示する店舗情報を取得
        $dbh = dbConnect();
        $sql = 'SELECT DISTINCT s.`id`, s.`shop_name`, s.`social_profile`, s.`prefecture_id`, s.`street`, s.`building`, s.`shop_img1`, c.`city_name`, cate.`category_name` FROM `shops` AS s LEFT JOIN `products` AS p ON s.`id` = p.`shop_id` LEFT JOIN `cities` AS c ON s.`city_id` = 6 LEFT JOIN `category` AS cate ON cate.`id` = p.`category_id` WHERE s.`prefecture_id` = :p_id AND s.`delete_flg` = 0';
        $data = array(':p_id' => $p_id);
        if(!empty($city_id)) {
            $sql .= ' AND s.`city_id` = :city_id';
            $data = array_merge($data, array(':city_id' => $city_id));
