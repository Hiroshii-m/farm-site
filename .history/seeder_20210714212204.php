...
try{
    ...
    for($i = 675; $i <= 736; $i++){
        ...
        $u_id = 2219 + $k;
        $c_id = $i;
        $data = array(':s_id' => $s_id, ':u_id' => $u_id, ':p_name' => '', ':p_detail' => '日本カボチャ、西洋カボチャ、ぺポカボチャの３種類に分けられ、現在の主流は、ほくほくして甘みの強い西洋カボチャです。', ':category_id' => 2, ':term' => '秋、夏。', ':p_value' => '', ':create_date' => date('Y-m-d H:i:s'));
        // クエリ実行
        queryPost($dbh, $sql, $data);
        header("Location:mypage.php");
    }
