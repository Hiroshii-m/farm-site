...
try{
    ...
    for($i = 0; $i <= 61; $i++){
        ...
        $u_id = 2220 + $i;
        $c_id = $i;
        $data = array(':s_id' => $s_id, ':u_id' => $u_id, ':p_name' => 'トマト', ':p_detail' => '稲の果実である籾から外皮を取り去った粒状の穀物である。穀物の一種として米穀（べいこく）とも呼ぶ。日本では主食の一つ。', ':category_id' => 2, ':term' => '春、夏。', ':p_value' => '100', ':p_number' => '1個', ':p_img' => 'uploads/b1aea79fd7b31fb9d156e203e6ca1640ac6d0891.png', ':create_date' => date('Y-m-d H:i:s'));
        // クエリ実行
        queryPost($dbh, $sql, $data);
        header("Location:mypage.php");
    }
